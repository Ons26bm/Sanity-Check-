...
#             WHERE (rslt.response_data->'$.Qualifications' = 'Client Résilié' or rslt.response_data->'$.Availability' in ("offtarget","wrongnumber") )
...
#         excel_file = r"C:\Users\Administrateur\Desktop\STAR\Auto_Report\PRP_FILES\NINGEN_STAR_Assurances_Campagne_PRP_wrong_number_.xlsx"
#         import pandas as pd
...
#     temp_excel_path = os.path.join(temp_folder, f"NINGEN_STAR_Assurances_Campagne_PRP_wrongnumber_{end_date}.xlsx")
...
#                 <p style="font-size: 16px;">Cet e-mail est envoyé dans le cadre du post-traitement de la campagne PRP STAR. Vous trouverez en pièce jointe l’extract post-production regroupant les clients résiliés, les faux numéros et les hors cible.</p>
...
#     mail.Subject = f'Partenariat STAR Assurances | NINGEN [PRP - Hors cible & Faux Numéros {start_date} au {end_date}]'
...
# Log file name
log_filename = f"PRP_wrong_number_OFF_TARGET_{datetime.now().strftime('%Y%m%d_%H.%M')}.txt"
...
def build_data(df_rslt: pd.DataFrame, df_inj: pd.DataFrame) -> pd.DataFrame:
    ...
    df_rslt_filtered = df_rslt[
        ((df_rslt["Qualifications"] == "Client Résilié") |
         ((df_rslt["Availability"] == "offtarget") | (df_rslt["Availability"] == "wrongnumber"))) &
...
def main():
    ...
    try:
        ...
        write_log(f"   - Lignes filtrées (Fiche avec wrongnumber) : {len(Data)}")
        ...
        temp_excel_path = os.path.join(
            temp_folder,
            f"NINGEN_STAR_Assurances_Campagne_PRP_wrongnumber_{end_date}.xlsx"
        )
        Data.to_excel(temp_excel_path, sheet_name='Data_ROG', index=False)
        ...
        try:
            ...
            mail.Subject = (
                ...
                f'[PRP - Hors cible & Faux Numéros {start_date} au {end_date}]'
            )
