...
#             JOIN datawarehouse.outbound_surveydata AS inj 
#                 ON rslt.surveydata_beside_id = inj.surveydata_beside_id 
#             WHERE rslt.response_data->'$.Qualifications' = 'Fiche avec MAJ'
#                 AND rslt.surveyschema_beside_id = 30
#                 AND inj.surveyschema_beside_id = 30
#                 AND rslt.created_at < CURDATE() - INTERVAL 1 DAY;
...
#         excel_file = r"C:\Users\Administrateur\Desktop\STAR\Auto_Report\PRP_FILES\NINGEN_STAR_Assurances_Campagne_PRP_.xlsx"
#         import pandas as pd
#         from datetime import datetime

#         # Convertir les dates en format datetime
#         date_debut = datetime(2025, 10, 25)
...
#         """
#         colonnes_ordre = ["ADRESSE","NUM_TEL_2","CODE_USAGE","CONVENTION","DATE_EFFET","CODE_AGENCE","EMAIL_TIERS","IDENTIFIANT","TYPE_CONTRAT","phone number",
#                           "NUMERO_CONTRAT","TYPE_IDENTIFIANT","DATE_EFFET_ORIGINE","NOM_PRENOM_ASSURE1","NOM_PRENOM_ASSURE2","Availability","Recall Date","Feedback",
#                           "Est-ce qu’on peut toujours vous contacter sur ce numéro ?","Numéro Corrigé","Vous avez un autre numéro ?","L’adresse qu’on a est toujours la même ?",
#                           "Adresse Corrigée","Vous avez une adresse mail ?","C’est vous l’assuré ou quelqu’un d’autre ?","Nom de l'assuré","Numéro de téléphone de l'assuré",
#                           "Tu as déjà entendu parler de MySTAR et tu l’utilises ?","Qualification SAT","Raison d'insatisfaction","Qualifications","Comment","Autre raison d'insatisfaction"
...
#     end_date = (date.today() - timedelta(days=2)).strftime("%d-%m-%Y")

#     # --- Création fichier Excel temporaire ---
#     temp_folder = os.path.join(os.getenv('TEMP'), 'survey_attachments')
#     os.makedirs(temp_folder, exist_ok=True)
#     temp_excel_path = os.path.join(temp_folder, f"NINGEN_STAR_Assurances_Campagne_PRP_{end_date}.xlsx")
...
#                 <p style="font-size: 16px;">Cet email est envoyé dans le cadre du post-traitement de la campagne PRP STAR, contenant en pièce jointe l'extract post-production des clients dont les coordonnées ont été mises à jour.</p>
...
#     mail.Subject = f'Partenariat STAR Assurances | NINGEN [PRP - Fiches avec MAJ - {start_date} au {end_date}]'
...
#     try:
#         app = Application(backend="uia").connect(title_re=f".*Partenariat STAR | NINGEN_ Suivi Traitement PRP {start_date}.*")
#         window = app.window(title_re=f".*Partenariat STAR | NINGEN_ Suivi Traitement PRP  {start_date}.*")
...
# Log file name
log_filename = f"PRP_MAJ_{datetime.now().strftime('%Y%m%d_%H.%M')}.txt"
# =====================
# SharePoint Helpers
# =====================
...
def build_data(df_rslt: pd.DataFrame, df_inj: pd.DataFrame) -> pd.DataFrame:
    ...
    df_rslt_filtered = df_rslt[
        (df_rslt["Qualifications"] == "Fiche avec MAJ") &
    ...
    colonnes_ordre = [
        "ADRESSE", "NUM TEL 2", "CODE USAGE", "CONVENTION", "DATE EFFET",
        "CODE AGENCE", "EMAIL TIERS", "IDENTIFIANT", "TYPE CONTRAT", "phone number",
        "NUMERO CONTRAT", "TYPE IDENTIFIANT", "DATE EFFET ORIGINE",
        "NOM PRENOM ASSURE1", "NOM PRENOM ASSURE2",
        "Availability", "Recall Date", "Feedback",
        "Est-ce qu’on peut toujours vous contacter sur ce numéro ?",
        "Numéro Corrigé", "Vous avez un autre numéro ?",
        "L’adresse qu’on a est toujours la même ?", "Adresse Corrigée",
        "Vous avez une adresse mail ?",
        "C’est vous l’assuré ou quelqu’un d’autre ?",
        "Nom de l'assuré", "Numéro de téléphone de l'assuré",
        "Tu as déjà entendu parler de MySTAR et tu l’utilises ?",
        "Qualification SAT", "Raison d'insatisfaction", "Qualifications",
        "Comment", "Autre raison d'insatisfaction"
    ]
    

    colonnes_finales = [col for col in colonnes_ordre if col in Data.columns]
    Data = Data[colonnes_finales]
    Data  = Data.rename(columns={"NUM TEL 2": "NUM_TEL_2",
                                      "CODE USAGE": "CODE_USAGE",
                                      "DATE EFFET": "DATE_EFFET",
                                      "CODE AGENCE": "CODE_AGENCE",
                                      "EMAIL TIERS": "EMAIL_TIERS",
                                      "IDENTIFIANT": "IDENTIFIANT",
                                      "TYPE CONTRAT": "TYPE_CONTRAT",
                                      "NUMERO CONTRAT": "NUMERO_CONTRAT",
                                      "TYPE IDENTIFIANT": "TYPE_IDENTIFIANT",
                                      "DATE EFFET ORIGINE": "DATE_EFFET_ORIGINE",
                                      "NOM PRENOM ASSURE1": "NOM_PRENOM_ASSURE1",
                                      "NOM PRENOM ASSURE2": "NOM_PRENOM_ASSURE2",
                                      })
    return Data
...
def main():
    ...
    try:
        ...
        write_log(f"   - Lignes filtrées (Fiche avec MAJ) : {len(Data)}")
        ...
        temp_excel_path = os.path.join(
            temp_folder,
            f"NINGEN_STAR_Assurances_Campagne_PRP_{end_date}.xlsx"
        )
        Data.to_excel(temp_excel_path, sheet_name='Data_ROG', index=False)
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;
                        background-color: #f9f9f9; box-shadow: 0 4px 8px rgba(0,0,0,0.1); position: relative;">
                <div style="position: absolute; top: 0px; left: 0px; padding: 5px;">
                ...
                <div style="margin-top: 60px;">
                    ...
                    <p style="font-size: 16px;">
                        Cet email est envoyé dans le cadre du post-traitement de la campagne PRP STAR,
                        contenant en pièce jointe l'extract post-production des clients dont les coordonnées
                        ont été mises à jour.
                    </p>
                </div>
                <p><strong>NINGEN Data Analytics</strong></p>
                <div style="margin-top: 20px; font-size: 12px; color: #666;">
                    <p>
                        Ceci est un message généré automatiquement. Merci de ne pas y répondre.<br>
                        <strong>Besoin d'assistance ?</strong><br>
                ...
                <div style="text-align: center; margin-top: 20px; font-size: 10px; color: #666;">
                    <p>
                        Ce message et les éventuelles pièces jointes sont strictement confidentiels et destinés
                        exclusivement au(x) destinataire(s) indiqué(s). Toute utilisation, diffusion ou reproduction
                        non autorisée est interdite. Si vous avez reçu ce message par erreur, merci d'en avertir
                        immédiatement l'expéditeur et de supprimer le courriel.
                    </p>
                </div>
            </div>
        </body>
        ...
        try:
            ...
            mail.Subject = (
                ...
                f'[PRP - Fiches avec MAJ - {start_date} au {end_date}]'
            )
