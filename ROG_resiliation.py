...
#         query1 = """
#         SELECT inj.data,rslt.response_data,rslt.created_at
#         FROM datawarehouse.outbound_surveyresults AS rslt
#         INNER JOIN datawarehouse.outbound_surveydata AS inj ON rslt.surveydata_beside_id = inj.surveydata_beside_id where rslt.surveyschema_beside_id in (29)and rslt.response_data->'$.Availability'='available';
#         """
...
#         excel_file = r"C:\Users\Administrateur\Desktop\STAR\Auto_Report\ROG_FILES\NINGEN_STAR_Assurances_Campagne_Résiliation_V1.2__ROG_.xlsx"
#         import pandas as pd
...
#         """
#         colonnes_ordre = [
#             'created_at','num_police', 'num_immatriculation', 'phone number', 'tel2', 
#             'avenant', 'date_effet_avenant', 'date_emission_avenant', 
#             'date_effet_avenant', 'code_agence_contrat', 'nom_pre_raison_social_ass',
#             'groupe de motif', 'motif de la résiliation', 'Availability', 
#             'Validation de la Résiliation', 'Raison annulation', 'Raison d\'annulation'
#             'Préciser si Autre', 'Si Proximité, Agence Cible', 
#             'Si Concurrence, quelle campagnie', 'Réclamation', 
#             'Réclamation Remontée', 'Type Réclamation', 'Autre type de réclamation',
#             'Commentaire Détails Réclamation', 'Feedback', 
#             'Qualification du contact','Adresse mail du client', 'Commentaire Général', 'Comment'
#         ]
#                 # Créer la liste finale des colonnes pour la réorganisation
#         colonnes_finales = []
        
#         # Ajouter d'abord les colonnes de l'ordre spécifié qui existent dans le DataFrame
...
#     temp_excel_path = os.path.join(temp_folder, f"NINGEN_STAR_Assurances_Campagne_Résiliation_V1.2__ROG_{end_date}.xlsx")
#     Data.to_excel(temp_excel_path, sheet_name='Data_ROG', index=False)
...
#     # --- Encode le logo ---
#     image_path = r"C:\Users\Administrateur\Desktop\STAR\Auto_Report\logo-complet-color.png"
...
#                 <img src="data:image/png;base64,{encoded_image}" alt="Logo" style="height: 50px; width: auto; object-fit: contain;">
#             </div>
#             <div style="margin-top: 60px;">
#                 <p style="font-size: 16px;">Bonjour,</p>
#                 <p style="font-size: 16px;">Veuillez trouver en pièce jointe la consolidation des ROG Flag pour la campagne Résiliation. </p>
...
#     mail.Subject = f'Partenariat STAR Assurances | NINGEN [ROG Résiliation {start_date} au {end_date}]'
#     #mail.To = "rz35n@ningen-group.com"
#     #mail.CC = "yq68h@ningen-group.com"
#     mail.To = "lilia.ferchichi@star.com.tn;Oussema.ABID@STAR.com.tn"
#     mail.CC = "faten.maazaoui@star.com.tn;comd@ningen-group.com;Ningen-pperformance@ningen-group.com;Ningen-Data-Management@ningen-group.com;o.benmaaouia@ningen-group.com"
...
#     # --- Connexion avec pywinauto et envoi automatique ---Partenariat STAR | NINGEN_ Suivi Traitement Résiliation 
#     try:
#         app = Application(backend="uia").connect(title_re=f".*Partenariat STAR | NINGEN_ Suivi Traitement Résiliation {start_date}.*")
#         window = app.window(title_re=f".*Partenariat STAR | NINGEN_ Suivi Traitement Résiliation  {start_date}.*")
...
RAW_DATA_PATH = "General/STAR/RESIL/raw_data"

FILE_RSLT = f"{RAW_DATA_PATH}/outbound_surveyresults1.xlsx"
...
# pylint: disable=duplicate-code
SURVEY_SCHEMA_ID   = 29  # ID du survey schema à filtrer dans les résultats
...
# Log file name
log_filename = f"Resiliation_ROG_{datetime.now().strftime('%Y%m%d_%H.%M')}.txt"
    ...
    with open(filename, "rb") as f:
        for i, line in enumerate(f, start=1):
            try:
                ...
                print(f"[UTF-8 ERROR] Ligne {i}: {e}", file=sys.stderr)
# ============================================================
...
def coalesce_duplicate_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    When the same column name appears multiple times (e.g. after flatten),
    merge them row-wise: take the first non-null value across all duplicates.
    Keeps a single column with the base name.
    """
    df = df.copy()
    cols = pd.Series(df.columns)
    # Find base names that appear more than once
    seen = {}
    for c in cols:
    ...
    for base, count in seen.items():
        if count > 1 and base not in processed:
            # Get all positional indices for this name
            positions = [i for i, c in enumerate(df.columns) if c == base]
            # Coalesce: first non-null wins
            merged = df.iloc[:, positions[0]]
            for pos in positions[1:]:
                merged = merged.combine_first(df.iloc[:, pos])
            # Drop all occurrences, re-insert once
            df = df.iloc[:, [i for i, c in enumerate(df.columns) if c != base]]
            df.insert(positions[0], base, merged)
            processed.add(base)
    return df
...
def merge_numbered_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Merge columns like:
        ...
        'Column (1)'
        'Column (2)'
    into a single column 'Column'
    """

    df = df.copy()
    columns = list(df.columns)

    # Map base name -> list of related columns
    col_groups = {}

    for col in columns:
        base = re.sub(r"\s\(\d+\)$", "", col)
        col_groups.setdefault(base, []).append(col)

    for base, cols in col_groups.items():
        if len(cols) > 1:
            # Merge columns row-wise (take first non-null value)
            df[base] = df[cols].bfill(axis=1).iloc[:, 0]
            # Drop all numbered columns except base
            cols_to_drop = [c for c in cols if c != base]
...
# Data Processing
# =====================
def merge_underscore_one_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Merge columns like:
        'Réclamation'
        'Réclamation_1'
    into a single column 'Réclamation'
    (row-wise: take first non-null value)
    """
    ...
    for col in columns:
        if col.endswith("_1"):
            base_col = col[:-2]  # remove "_1"

            if base_col in df.columns:
                # Merge row-wise (take first non-null)
                df[base_col] = df[[base_col, col]].bfill(axis=1).iloc[:, 0]
...
def build_data(df_rslt: pd.DataFrame, df_inj: pd.DataFrame) -> pd.DataFrame:
    ...
    # e.g. "data ADRESSE" → "ADRESSE"
    rename_inj = {
        col: col.replace("data ", "").strip()
        for col in df_inj.columns
        if col.startswith("data ")
    }
    df_inj = df_inj.rename(columns=rename_inj)

    # ── Apply SQL WHERE filters ─────────────────────────────────────────────
    cutoff = pd.Timestamp(date.today() - timedelta(days=1))

    df_rslt_filtered = df_rslt[
        (
         (df_rslt["Availability"] == "available") ) &
        (df_rslt["created at"] < cutoff)
    ].copy()
    ...
    # df_rslt_filtered = merge_numbered_columns(df_rslt_filtered)
    # df_inj_filtered = merge_numbered_columns(df_inj_filtered)
    
    # df_rslt_filtered = merge_underscore_one_columns(df_rslt_filtered)
    # df_inj_filtered = merge_underscore_one_columns(df_inj_filtered)
    ...
    Data =merge_numbered_columns(Data)
    Data = merge_underscore_one_columns(Data)
    
    # ── Rename "phone number" from inj (may already be present after prefix strip) ─
    # If both tables had a "created at" column they are now "created at_rslt" / "created at_inj"
    Data['created at'] = pd.to_datetime(Data['created at_rslt'], dayfirst=True)
    # ── Column order matching original script ───────────────────────────────
    colonnes_ordre = [
        'created at','num police', 'num immatriculation', 'phone number', 'tel2', 
            'avenant', 'date effet avenant', 'date emission avenant', 
            'date effet avenant', 'code agence contrat', 'nom pre raison social ass',
            'groupe de motif', 'motif de la résiliation', 'Availability', 
            'Validation de la Résiliation', 'Raison annulation', 'Raison d\'annulation'
            'Préciser si Autre', 'Si Proximité, Agence Cible', 
            'Si Concurrence, quelle campagnie', 'Réclamation', 
    ...
    colonnes_finales = [col for col in colonnes_ordre if col in Data.columns]
    Data = Data[colonnes_finales]
    Data  = Data.rename(columns={
                                 "num police": "num_police",
                                 "num immatriculation":"num_immatriculation",
                                 "date effet avenant":"date_effet_avenant",
                                 "date emission avenant":"date_emission_avenant",
                                 "code agence contrat":"code_agence_contrat",
                                    "nom pre raison social ass":"nom_pre_raison_social_ass"
                                       
                                      })
    return Data
def remove_trailing_number_suffix(df: pd.DataFrame) -> pd.DataFrame:
    """
    Supprime ' (1)', ' (2)', ... uniquement à la fin des noms de colonnes.
    Exemple:
        'Date Injection (1)' -> 'Date Injection'
        'Column (23)' -> 'Column'
    """
    df = df.copy()
    df.columns = [
...
def main():
    ...
    try:
        ...
        import os
        check_utf8(os.path.abspath(__file__))
        # DB via SSH
        tunnel = None
        conn = None
        try:
            ...
            df_existing = merge_numbered_columns(df_existing)
            df_new = merge_numbered_columns(df_new)
            
            df_existing.columns = make_unique(df_existing.columns.tolist())
            df_new.columns = make_unique(df_new.columns.tolist())
        ...
        try:
            ...
            def normalize_key_values(series: pd.Series) -> pd.Series:
                return (
                    series.astype("string")
                    .str.strip()
                    .str.replace(".0", "", regex=False)  # remove Excel float effect
                )
            
            # Normalize both sides
            existing_keys = set(
                normalize_key_values(df_existing1[key_col_existing1])
                .dropna()
                .tolist()
            )
            
            db_keys_series1 = normalize_key_values(df_db_aligned1[key_col_existing1])
            ...
            mask_new1 = ~db_keys_series1.isin(existing_keys)
            ...
            df_existing1 = df_existing1.loc[:, ~df_existing1.columns.duplicated()]
            df_new1 = df_new1.loc[:, ~df_new1.columns.duplicated()]

            # Re-align new data to existing columns strictly
            df_new1 = df_new1.reindex(columns=df_existing1.columns)    
        ...
        write_log(f"   - Lignes filtrées  : {len(Data)}")
        ...
        temp_excel_path = os.path.join(
            temp_folder,
            f"NINGEN_STAR_Assurances_Campagne_Résiliation_ROG_{end_date}.xlsx"
        )
        Data.to_excel(temp_excel_path, sheet_name='Data_ROG', index=False)
        ...
        try:
            ...
            mail = outlook.CreateItem(0)
            mail.Subject = (
                f'Partenariat STAR Assurances | NINGEN '
                f'[ROG Résiliation {start_date} au {end_date}]'
            )
            ...
            # mail.To = "lilia.ferchichi@star.com.tn;Oussema.ABID@STAR.com.tn"
            # mail.CC = (
            #     "faten.maazaoui@star.com.tn;comd@ningen-group.com;"
            #     "Ningen-pperformance@ningen-group.com;"
            #     "Ningen-Data-Management@ningen-group.com;"
