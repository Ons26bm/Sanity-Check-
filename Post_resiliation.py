...
#         INNER JOIN datawarehouse.outbound_surveydata AS inj ON rslt.surveydata_beside_id = inj.surveydata_beside_id where rslt.surveyschema_beside_id in (29);
#         """
...
#     temp_excel_path = os.path.join(temp_folder, f"PostProd_STAR_Res_{end_date}.xlsx")
...
#                 <p style="font-size: 16px;">Veuillez trouver ci-joint l'extract post Production de l'activité STAR de la campagne Résiliation pour la période du 
...
#     mail.Subject = f' Post Prod STAR-Résiliation du {start_date} au {end_date}'
...
# Load environment
# =====================
#load_dotenv(dotenv_path=r"C:\Users\Administrateur\Desktop\STAR\Post Prod\STAR\Post Prod\env 9F")
...
# Log file name
log_filename = f"Post_Prod_Resiliation_{datetime.now().strftime('%Y%m%d_%H.%M')}.txt"
...
def clean_dataframe_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = remove_trailing_number_suffix(df)
    return df
...
def main():
    ...
    try:
        ...
        try:
            ...
            # df_existing1 = deduplicate_columns(df_existing1)
            # df_new1 = deduplicate_columns(df_new1)   
        ...
        temp_excel_path = os.path.join(
            temp_folder,
            f"PostProd_STAR_Res_{end_date}.xlsx"
        )
        Data.to_excel(temp_excel_path, sheet_name='Data_N', index=False)
        ...
        try:
            ...
            mail = outlook.CreateItem(0)
            mail.Subject = (
                f'Post Prod STAR-Résiliation du '
